...
function getChart(ap_id, chart_id) {
  ...
  var url = "/data/" + ap_id;
  // show loader
  var loader = __WEBPACK_IMPORTED_MODULE_0_jquery___default()(".load-wrapp");
  loader.show();
  __WEBPACK_IMPORTED_MODULE_0_jquery___default.a.ajax({url: url}).done(function( data ) {
    // hide loader
    loader.hide();
    // draw chart
...
function drawChart(data_obj, id) {
  __WEBPACK_IMPORTED_MODULE_0_jquery___default.a.getJSON("/charts.json", function(charts) {
    var graph = charts[id];
...
function buildArray(stations, columns, callback) {
  ...
  var head = [];
  var rowFields = [];
  __WEBPACK_IMPORTED_MODULE_0_jquery___default.a.each(columns, function( index, c) {
    head.push(c.cfg);
  });
  a.push(head);

  // insert values from stations based on values array
  __WEBPACK_IMPORTED_MODULE_0_jquery___default.a.each(stations, function( index, obj ) {
    var row = [];
    __WEBPACK_IMPORTED_MODULE_0_jquery___default.a.each(columns, function( index, c ) {
      if (Array.isArray(c.value)) {
        var x = obj;
        __WEBPACK_IMPORTED_MODULE_0_jquery___default.a.each(c.value, (i, v)=>{
          x = x[v];
        });
        row.push(x);
      } else {
